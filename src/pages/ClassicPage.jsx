...
function ClassicPage({ guesses, setGuesses }) {
  ...
  return (
    <div>
      ...
      <div style={{ margin: '24px auto', maxWidth: 800, fontSize: 18, background: '#f5f5f5', borderRadius: 8, padding: 18, border: '1px solid #ddd', whiteSpace: 'pre-line' }}>
        <form
          onSubmit={e => {
            e.preventDefault();
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', fontWeight: 600, gap: 11, marginBottom: 8, alignItems: 'center' }}>
          <div style={{ minWidth: 80, textAlign: 'center' }}>Pokemon</div>
          <div style={{ minWidth: 80, textAlign: 'center' }}>Color</div>
          <div style={{ minWidth: 80, textAlign: 'center' }}>Types</div>
          <div style={{ minWidth: 80, textAlign: 'center' }}>Habitat</div>
          <div style={{ minWidth: 80, textAlign: 'center' }}>Height (m)</div>
          <div style={{ minWidth: 80, textAlign: 'center' }}>Weight (kg)</div>
        </div>
        <div>
          {guesses.map((poke, idx) => {
            ...
            const ArrowIcon = ({ dir }) => {
              ...
              if (dir === 'up') {
                return (
                  <svg width="32" height="32" viewBox="0 0 32 32" style={{ display: 'inline', verticalAlign: 'middle' }}>
                    {/* Rectangle base */}
            ...
            return (
              <div key={poke.name + idx} className="feedback-grid" style={{ gridTemplateColumns: 'repeat(6, 48px)' }}>
                <div className="feedback-box feedback-pokemon-box">
                  <img
                    ...
                    alt={poke.name}
                    style={{ width: 72, height: 72, objectFit: 'contain', display: 'block', margin: '0 auto', transform: 'scale(1.2)' }}
                ...
                <div className={`feedback-box ${heightStatus}`} style={{ position: 'relative' }}>
                  {cmp.height !== 'match' && (
                    <svg width="100" height="100" viewBox="0 0 100 100" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0.2, pointerEvents: 'none' }}>
                      {cmp.height === 'up' ? (
                        <>
